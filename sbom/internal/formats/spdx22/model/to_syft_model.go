...
func findLinuxReleaseByPURL(doc *spdx.Document) *linux.Release {
	for _, p := range doc.Packages {
		...
		if err != nil {
			...
			fmt.Printf("unable to parse purl: %s", purlValue)
			continue
		}
...
func toFileMetadata(f *spdx.File) (meta source.FileMetadata) {
	...
	for _, typ := range f.FileTypes {
		switch spdxhelpers.FileType(typ) {
		case spdxhelpers.ImageFileType:
			meta.MIMEType = "image/"
		case spdxhelpers.VideoFileType:
			meta.MIMEType = "video/"
		case spdxhelpers.ApplicationFileType:
			meta.MIMEType = "application/"
		case spdxhelpers.TextFileType:
			meta.MIMEType = "text/"
		case spdxhelpers.AudioFileType:
			meta.MIMEType = "audio/"
		case spdxhelpers.BinaryFileType:
		case spdxhelpers.ArchiveFileType:
		case spdxhelpers.OtherFileType:
		}
	}
	return meta
}
...
func toSyftRelationships(spdxIDMap map[string]interface{}, doc *spdx.Document) []artifact.Relationship {
	...
	for _, r := range doc.Relationships {
		...
		if r.RefA.DocumentRefID != "" && requireAndTrimPrefix(r.RefA.DocumentRefID, "DocumentRef-") != string(doc.SPDXIdentifier) {
			...
			fmt.Printf("ignoring relationship to external document: %+v", r)
			continue
		}
		...
		if !fromOk || !(toPackageOk || toLocationOk) {
			...
			fmt.Printf("unable to find valid relationship mapping from SPDX 2.2 JSON, ignoring: (from: %+v) (to: %+v)", a, b)
			continue
		}
		var to artifact.Identifiable
		var typ artifact.RelationshipType
		if toLocationOk {
			if r.Relationship == string(spdxhelpers.ContainsRelationship) {
				typ = artifact.ContainsRelationship
				to = toLocation
			}
		} else {
			switch spdxhelpers.RelationshipType(r.Relationship) {
			case spdxhelpers.ContainsRelationship:
				typ = artifact.ContainsRelationship
				to = toPackage
			case spdxhelpers.OtherRelationship:
	...
	case pkg.GoModulePkg:
		...
		for _, value := range p.PackageChecksums {
			...
			if err != nil {
				...
				fmt.Printf("invalid h1digest: %v %v", value, err)
				continue
			}
			h1Digest = digest
			break
...
func findPURLValue(p *spdx.Package) string {
	for _, r := range p.PackageExternalReferences {
		if r.RefType == string(spdxhelpers.PurlExternalRefType) {
			return r.Locator
		}
	}
	return ""
...
func extractCPEs(p *spdx.Package) (cpes []cpe.CPE) {
	for _, r := range p.PackageExternalReferences {
		if r.RefType == string(spdxhelpers.Cpe23ExternalRefType) {
			c, err := cpe.New(r.Locator)
			if err != nil {
				// log.Warnf("unable to extract SPDX CPE=%q: %+v", r.Locator, err)
				fmt.Printf("unable to extract SPDX CPE=%q: %+v", r.Locator, err)
				continue
			}
			cpes = append(cpes, c)
		}
...
func parseLicense(l string) []string {
	if l == spdxhelpers.NOASSERTION || l == spdxhelpers.NONE {
		return nil
	}
