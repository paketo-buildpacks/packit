...
func testSBOM(t *testing.T, context spec.G, it spec.S) {
	...
	context("GenerateFromDependency", func() {
		it("generates a SBOM from a dependency for latest schema versions", func() {
			...
			Expect(goArtifact.Licenses[0].Value).To(Equal("BSD-3-Clause"), syft.String())
			Expect(goArtifact.CPEs).To(ContainElement(cpe{
				CPE:    "cpe:2.3:a:golang:go:1.16.9:*:*:*:*:*:*:*",
				Source: "declared",
			}), syft.String())
			...
			Expect(syftDefaultOutput.Source.Metadata.Path).To(Equal("some-path"), syft.String())
			...
			Expect(goPackage.LicenseConcluded).To(Equal("NOASSERTION"), spdx.String())
		...
		context("when the input dependency does not have a CPE or a PURL", func() {
			it("succeeds in generating an SBOM without CPEs", func() {
				...
				Expect(goArtifact.Name).To(Equal("Go"), syft.String())
				Expect(goArtifact.Version).To(Equal("1.16.9"), syft.String())
				Expect(goArtifact.Licenses).To(ContainElement(syftLicense{Value: "BSD-3-Clause"}), syft.String())
				Expect(syftDefaultOutput.Source.Type).To(Equal("directory"), syft.String())
				Expect(syftDefaultOutput.Source.Metadata.Path).To(Equal("some-path"), syft.String())
				Expect(goArtifact.PURL).To(BeEmpty())
				Expect(goArtifact.CPEs).To(ContainElement(cpe{CPE: "cpe:2.3:-:-:-:-:-:-:-:-:-:-:-", Source: "declared"}))
		...
		context("when the input dependency has CPEs and CPE", func() {
			it("uses CPEs, not CPE", func() {
				...
				Expect(syftDefaultOutput.Source.Metadata.Path).To(Equal("some-path"), syft.String())
				Expect(goArtifact.PURL).To(BeEmpty())
				Expect(goArtifact.CPEs).To(ContainElements(cpe{CPE: "cpe:2.3:a:some:other:cpe:*:*:*:*:*:*:*", Source: "declared"}, cpe{CPE: "cpe:2.3:a:another:cpe:to:include:*:*:*:*:*:*", Source: "declared"}))
			})
		})
	...
	context("InFormats", func() {
		context("failure cases", func() {
			...
			context("when a requested version is not supported", func() {
				it("returns an error", func() {
					_, err := sbom.SBOM{}.InFormats(fmt.Sprintf("%s;version=0.0.0", sbom.CycloneDXFormat))
					Expect(err).To(MatchError(fmt.Sprintf(`version '0.0.0' is not supported for SBOM format '%s'`, sbom.CycloneDXFormat)))
				})
			})
		})
	})
